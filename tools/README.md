...
| `gen_synthetic_night.py` | Generates a realistic night in the exact firmware schema; `--truth` emits the **ground-truth hypnogram** and `--hrv` populates the optional `rmssd_ms` column and sets the `HRV_VALID` flag bit, so the scorer's accuracy is measurable without a real overnight run. |
